...
module.exports = (function () {
  ...
  find: function(collection, criteria, callback) {
    ...
    transaction.exec(callback, function(done) {
      ...
      redis.smembers(index, function(err, members) {
        ...
        if(present(criteria.limit)){
            members = members.slice(criteria.limit)
        }
  ...
  update: function(collection, criteria, values, callback) {
    ...
    transaction.exec(callback, function(done) {
      ...
      redis = this;
      match = utils.matchFn({where:criteria});
      index = transaction.indexKey(collection, primary);

      redis.smembers(index, function(err, members) {
        if(err) return done(err);
        if(!members) return done();
        redis.mget(members,function(err, member){
          if(err) done(err);
          
          var ex = null;
          var up = [];

          var unique_test = false;
          
          unqiue = schema.unique(collection);
          unique.forEach(function(item){              
              unique_test = unique_test|utils.present(values[item]);
          });

          // wost case is faster if it's syncronous
          for(i in member){
            
            data = schema.parse(collection, member[i]);

            if(!match(data)){
                if (unique_test){
                    ex = {message:adapterErrors.notUnique.message};
                }
                break;
            } 
            up.push(function(){
                        update(data, values, function(err, model) {
                            if(err) return callback(err);
                            models.push(model);
                            callback();
                        })
